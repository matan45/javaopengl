...
import flare.openglObjects.Query;
...
public class FlareRenderer {
	...
	private static final float TEST_QUAD_HEIGHT = TEST_QUAD_WIDTH
			* (float) (WindowManager.getWindow("main").getWidth() / WindowManager.getWindow("main").getHeight());
	private static final float TOTAL_SAMPLES = (float) Math
			.pow(TEST_QUAD_WIDTH * WindowManager.getWindow("main").getWidth() * 0.5f, 2) * 4;
	...
	private void renderFlare(FlareTexture flare) {
		...
		float yScale = xScale * (float) WindowManager.getWindow("main").getWidth()
